    ...
    for player in range(num_players):
        if sdl_joy[player]:
            for m in range(MAX_MAPPINGS):
                # Action keys
                if sdl_joy[player].get_pressed()[pygame.K_w]:
                    padcache[player][0] = 1
                else:
                    padcache[player][0] = 0

                 # Button B - jump
                if sdl_joy[player].get_pressed()[pygame.K_a]:
                    padcache[player][1] = 1
                else:
                    padcache[player][1] = 0

                 # Button B - jump
                if sdl_joy[player].get_pressed()[pygame.K_d]:
                    padcache[player][8] = 1
                else:
                    padcache[player][8] = 0

                 # Button B - jump
                if sdl_joy[player].get_pressed()[pygame.K_s]:
                    padcache[player][9] = 1
                else:
                    padcache[player][9] = 0

                # Movement buttons
                if sdl_joy[player].get_pressed()[pygame.K_UP]:
                    padcache[player][4] = 1
                else:
                    padcache[player][4] = 0

                if sdl_joy[player].get_pressed()[pygame.K_DOWN]:
                    padcache[player][5] = 1
                else:
                    padcache[player][5] = 0

                if sdl_joy[player].get_pressed()[pygame.K_LEFT]:
                    padcache[player][6] = 1
                else:
                    padcache[player][6] = 0

                if sdl_joy[player].get_pressed()[pygame.K_RIGHT]:
                    padcache[player][7] = 1
                else:
                    padcache[player][7] = 0

                # Start Button
                if sdl_joy[player].get_pressed()[pygame.K_f]:
                    padcache[player][3] = 1
                else:
                    padcache[player][3] = 0
                ...
                # Select Button
                if sdl_joy[player].get_pressed()[pygame.K_t]:
                    padcache[player][2] = 1
                else:
                    padcache[player][2] = 0
